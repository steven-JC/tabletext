    ...
    const TextTable = {
        ...
        parse: function(tableString, decode) {
            ...
            const keys = strRows
                ...
                .split(split[0])
                .map((item) =>
                    decode && item.trim() ? unescape(item.trim()) : item.trim()
                )
        ...
        stringify: function(plainObjectArray, encode) {
            let keysStr = Object.keys(plainObjectArray[0])
                .map((item) => (encode ? escape(item) : item))
                .join(',')
            const strRows = plainObjectArray.map((item) => {
                return Object.keys(item)
                    .map((key) => (encode ? escape(item[key]) : item[key]))
                    .join(',')
            })
        ...
        validate: function(tableString, decode) {
            ...
            if (
                split.some(
                    (item) => item.charCodeAt(0) > 127 || /\w/.test(item)
                )
            ) {
                return {
                    valid: false,
                    message:
                        '每行第一个字符为分隔符，应为英文符号，如：,|$%#@!&/ 等'
                }
            }
            const keys = strRows
                .shift()
